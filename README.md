...
## 📝 最新更新（v0.5.8，2025-11-07）

**🎨 完整的多模板管理系统 - FormTemplate 表与前后端全栈实现**

- ✅ **多模板支持** - 每个实体可以创建多个命名模板（如"客户详情-简版"、"完整版"）
- ✅ **默认模板机制** - 用户默认 > 系统默认 > 第一个模板的三级优先级
- ✅ **实体类型锁定** - 模板保存后自动锁定实体类型，防止误操作
- ✅ **删除保护** - 系统默认、用户默认、正在使用的模板不可删除
- ✅ **分组视图** - 支持平铺列表和按实体类型分组两种展示方式
- ✅ **完整 CRUD API** - 6个端点完整支持模板增删改查操作

**技术实现**：
- 后端：FormTemplate 域模型 + 数据库迁移 + TemplateEndpoints（378行）
- 前端：Templates.razor 完全重写（602行）+ FormDesigner 增强（1115行）
- 数据库：3个复合索引优化查询性能
- 向后兼容：保留 UserLayout API 作为回退方案
- 测试通过：创建/编辑/删除/默认设置/分组视图全部正常
...
详见：[实体自定义与发布系统设计文档](docs/实体自定义与发布系统设计文档.md)
