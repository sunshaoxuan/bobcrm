...
- **目录**：按领域划分（如 `01-module-a/`、`02-module-b/`）
- **文件名**：`TC-{DOMAIN}-{NNN}-{中文名称}.md`
  - `{DOMAIN}`：领域缩写（如 `AUTH`、`USER`、`SYS`）
  - `{NNN}`：三位序号（如 `001`、`010`）

示例：`docs/test-cases/03-module/TC-MOD-001-功能测试.md`

## 3. 语言规范

- **正文**: 必须使用 **简体中文**。
- **专业术语**: 保留英文原文 (如 `Widget`, `Endpoint`, `API`, `Framework`)，无需强行翻译。
...
- **列表**: 使用 `-` 或 `1.` 进行列表排版。
- **代码块**: 使用反引号 ``` 包裹代码，并指定语言 (如 `csharp`, `json`, `text`)。
...
- **禁止直接使用物理 SQL**：在描述数据模型或结构变更时，严禁直接贴入特定数据库（如 MySQL/PostgreSQL）的 SQL 语句。
- **使用逻辑 Schema 表达**：应使用 Markdown 表格描述逻辑字段、类型（逻辑类型如 String, UUID, Decimal）和约束。
