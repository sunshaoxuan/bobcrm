...
- 字段清单（全部标记为“接口字段”，禁止手工删除或修改数据类型）：
  1. `OrganizationId` (`Guid`, Required)：引用组织档案主键，用于授权与聚合过滤；
  2. `OrganizationCode` (`String(64)`, Required)：缓存组织编码，方便导出/对账；
  3. `OrganizationName` (`String(200)`, Optional)：默认由后台同步，可由业务层覆盖；
  4. `OrganizationPathCode` (`String(128)`, Required)：冗余 `PathCode` 以支持树型筛选、权限裁剪。
- 数据库 DDL 自动生成对应列及索引：`OrganizationId` 建立外键指向 `OrganizationNodes(Id)`，并附带 `IX_<表>_OrganizationId`、`IX_<表>_OrganizationPathCode`；`OrganizationCode`、`OrganizationName` 则作为普通列用于查询展示。
- 代码生成器输出新接口 `IOrganizational`（包含四个属性），实体类在勾选后会实现该接口，保持 OOP 单一职责；接口定义与其他接口一同写入 `GeneratedInterfaces.cs`。
- 前端行为：在实体定义页面的“接口”卡片中加入 “组织维度（IOrganizational）” Checkbox，提示文案阐明“将记录绑定到组织树，实现分组织管理”；字段表中会自动插入并锁定 4 个字段，取消勾选会立即移除。
- 业务约束：只有需要按组织隔离/授权的实体才勾选本接口；一旦实体已发布且拥有组织数据，需管理员权限才能取消或调整，避免跨聚合数据失真。
- 与组织管理联动：API 在保存实体记录时会校验 `OrganizationId` 是否存在，并自动填充 `OrganizationCode/Name/PathCode`，也可暴露钩子供低代码流程调用，实现“选择接口 → 拥有组织能力”的最佳实践体验。
