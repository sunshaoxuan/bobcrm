...
### 6.3 组织维度接口（IOrganizational）
- 增加新接口枚举 `EntityInterfaceType.Organization`，与 Base/Audit/Version 同级。勾选后自动注入组织维度字段，并在发布后锁定，确保聚合的组织语义不被破坏。
- 字段清单（全部标记为“接口字段”，禁止手工删除或修改数据类型）：
  1. `OrganizationId` (`Guid`, Required)：引用组织档案主键，用于授权与聚合过滤；实体层不再缓存 Code/Name/PathCode，避免冗余同步问题，这些信息由服务层按需装饰。
- 数据库 DDL 自动生成外键列：`OrganizationId` 建立外键指向 `OrganizationNodes(Id)`，并附带 `IX_<表>_OrganizationId` 索引，供权限和查询使用。
- 代码生成器输出新接口 `IOrganizational`（仅含 `OrganizationId` 属性），实体类在勾选后会实现该接口，保持 OOP 单一职责。
- 前端行为：在实体定义页面的“接口”卡片中加入 “组织维度（IOrganizational）” Checkbox，提示文案阐明“将记录绑定到组织树，实现分组织管理”；字段表中会自动插入并锁定 `OrganizationId`，取消勾选会立即移除。
- 业务约束：只有需要按组织隔离/授权的实体才勾选本接口；一旦实体已发布且拥有组织数据，需管理员权限才能取消或调整，避免跨聚合数据失真。
- 与组织管理联动：API 在保存实体记录时会校验 `OrganizationId` 是否存在，必要时通过组织服务加载 Code/Name/PathCode 并注入 DTO，不在实体表中冗余存储。

## 7. 组织档案（BASE 域）设计
- 引入固定归属领域 BASE，所有组织实体命名空间遵循 `BobCrm.BASE.{EntityName}`，同时允许用户扩展字段/子实体。
- 树形结构：每个企业仅有一个根节点，向下按层级无限扩展，节点字段包含 Id/Code/Name/ParentId/Level/PathCode/SortOrder。
- PathCode 采用每层 2 位递增编码（如 01、01.02、01.02.03），用于递归查询和排序；聚合根负责生成与维护，禁止用户手工修改。
- UI 采用“左树右表”+ 行内编辑模式：左侧展示层级树，右侧编辑节点属性，支持新增/删除/移动节点，并自动更新 PathCode 与 SortOrder。
- API/规则：提供获取整棵树、按 PathCode 查询子树、创建/更新/移动/删除节点接口；根节点禁止删除，删除前需先处理子节点或执行批量迁移。
- 最佳实践：组织聚合通过领域服务统一校验（同级 Code 唯一、禁止循环引用），并可触发领域事件用于权限与缓存更新，符合 OOP 单一职责与 Clean Architecture。

### 7.1 领域建模
- 聚合根 `OrganizationNode` 负责维护整棵树的不变量；服务 `OrganizationService` 封装新增、更新、删除操作。
- 字段说明：`Code`（同级唯一）、`Name`、`ParentId`、`Level`、`PathCode`（`xx.xx`）、`SortOrder`、`CreatedAt/UpdatedAt`。
- `PathCode` 通过“父节点 PathCode + 当前段（2 位数字）”生成，段值由兄弟节点最大值 + 1 得出，确保全局有序。
- 删除限制：有子节点时禁止删除；根节点不可删除；未来支持批量移动时需通过领域服务保证 PathCode 连锁更新。

### 7.2 API 合约
| Method | Route | Body | 说明 |
| --- | --- | --- | --- |
| `GET` | `/api/organizations/tree` | - | 返回整棵树（扁平加载后在内存组装），用于初始化页面。 |
| `POST` | `/api/organizations` | `CreateOrganizationRequest { ParentId?, Code, Name }` | 创建节点。`ParentId` 为空表示根节点，仅允许运营管理员创建一次。 |
| `PUT` | `/api/organizations/{id}` | `UpdateOrganizationRequest { Code, Name }` | 更新同级唯一编码与名称，不允许直接修改层级。 |
| `DELETE` | `/api/organizations/{id}` | - | 删除叶子节点。若存在子节点则返回 400 并提示“请先删除子节点”。 |

校验规则统一在服务层抛出 `InvalidOperationException`，REST 层包装为 400 + message，前端统一弹出 message。

### 7.3 前端交互
- 页面路径 `/organizations`，挂载在左侧 rail 菜单（一级菜单“组织关系”）。
- 布局：左树（`Tree`/自绘列表）展示层级，点击节点切换右侧“子节点表格”；右表沿用实体字段表格样式（行内编辑 + 保存/删除按钮）。
- 新增：根节点按钮仅在当前无根节点时启用。子节点按钮随选中节点变化，默认插入一行空数据。
- 交互原则：输入框获得焦点时即可编辑；保存成功后刷新局部树并保持当前选中；删除操作用 `Popconfirm` 防误触。
- 多语言：所有标签/消息写入 `I18nService` fallback，并同步数据库脚本，确保 zh/ja/en 一致。

### 7.4 测试与监控
- 单元测试覆盖：`OrganizationService` 的路径编码生成、同级唯一约束、根节点唯一性、删除限制。
- 集成测试：通过 `TestWebAppFactory` 调用端点验证 401/403、正常 CRUD、错误消息。
- 前端交互验证：新增/编辑/删除/折叠/刷新流程在 Playwright 或 BUnit 中至少覆盖一个 happy path；若暂不引入 UI 自动化，需要在 QA checklist 中列明手测步骤。
- 指标：后续可在 `OrganizationService` 中添加领域事件，发往审计日志或缓存失效逻辑，方便权限系统引用。

## 8. 角色与权限框架（Role Access Profile）
在组织维度基础上引入轻量化的角色权限体系，核心思路是“角色绑定组织 → 角色配置功能 + 数据范围 → 用户分配角色”。整体设计如下。

### 8.1 核心对象
| 对象 | 说明 |
| --- | --- |
| `RoleProfile` | 角色档案聚合根。字段：`Id`、`OrganizationId`、`Code`、`Name`、`Description`、`IsSystem`、`IsEnabled`、`SortOrder`、`ConcurrencyStamp`。负责聚合子对象并保护不可变条件。 |
| `RoleFunction` | 角色与功能树节点的关联。字段：`RoleId`、`FunctionId`、`Action`（View/Edit/Approve…）、`Access`（Allow/Deny/Inherit）。 |
| `RoleDataScope` | 角色的数据范围定义。字段：`RoleId`、`EntityName`、`ScopeType`（All/Self/Organization/OrganizationSubTree/Custom）、`FilterExpression`（OData/DSL）、`Remarks`。 |
| `RoleAssignment` | 用户-角色-组织的绑定。字段：`UserId`、`RoleId`、`OrganizationId`、`ValidFrom/ValidTo`、`IsPrimary`。 |

### 8.2 功能树（Function Tree）
- `FunctionNode`：`Id`、`ParentId`、`Code`、`DisplayName`、`Route`、`Actions`、`Order`、`Icon`。
- 支持分层维护与多语言，UI 采用树+勾选/半选来表示功能授权，细粒度动作通过 Tag 或多选框呈现。
- 角色编辑时可批量勾选节点或某一分支，保存到 `RoleFunction`。

### 8.3 数据范围（Data Scope）
- 每条配置描述“角色在某实体上的可见条件”，结构为 `EntityName + ScopeType + Condition`。
- 常见类型：
  1. `All`：全量数据；
  2. `Self`：仅本人拥有/创建的数据；
  3. `Organization`：当前组织；
  4. `OrganizationSubTree`：当前组织及子树；
  5. `Custom`：通过 DSL（如 `Status == "Active" && Region == CurrentRegion`）定义条件。
- 条件最终转换为表达式树或 OData filter，在查询/命令管道里自动注入。
- 默认种子（应用启动时自动写入）：
  1. **功能树**：预置树形节点覆盖现有主导航，例如 `APP.ROOT`（顶级）、`APP.DASHBOARD`、`APP.CUSTOMERS`、`APP.ENTITY`、`APP.TEMPLATES`、`APP.ORGANIZATIONS`、`APP.SETTINGS` 等，以及关键操作节点（`APP.CUSTOMERS.CREATE`、`APP.CUSTOMERS.EDIT` 等）。这些节点既驱动菜单/面包屑，也构成权限授权粒度。
  2. **系统管理员角色**：`SYS.ADMIN` 默认存在，绑定所有功能节点并附加 `("*", ScopeType = All)` 的数据范围，确保具备全功能/全数据权限。
  3. **管理员用户绑定**：检测到用户名为 `admin` 或邮箱 `admin@local` 的账户时，自动在 `RoleAssignments` 写入该用户与 `SYS.ADMIN` 的关联（`OrganizationId = null`），防止初始化后出现“无角色”的断链。
  4. 上述流程由 `AccessService.SeedSystemAdministratorAsync` 每次启动执行，具备幂等性：已有数据时只同步名称/路由，不会重复插入。

### 8.4 鉴权流程
1. 用户登录并选取目标组织（默认最近一次组织）。
2. 查找 `RoleAssignment` 中 `(UserId, OrganizationId)` 对应的角色集合。
3. 聚合角色的 `RoleFunction`，按 Allow/Deny/Inherit 合并动作，形成“功能授权集”。
4. 聚合 `RoleDataScope`，构建实体级过滤器；若同一实体出现多个 scope，则按“最严格优先”或显式策略（例如 All 覆盖 Self）。
5. 缓存 `(UserId, OrganizationId)` 维度的授权结果；当角色、功能树或分配变更时发事件失效缓存。

### 8.5 API 与前端
- **API**：角色档案 CRUD、功能树管理、角色分配、批量导入导出。
- **UI**：角色列表 → 编辑页面（基础信息 + 组织选择 + 功能树 Tab + 数据范围 Tab + 用户分配 Tab），场景化提示（如“组织必填”“系统角色不可跨组织”）。
- **校验**：同一用户在同一组织可拥有多个角色，权限取并集；组织切换时重新计算上下文。

该框架保持 OOP 聚合边界清晰，支持最常见的“组织 + 角色 + 功能 + 数据范围”授权模式，便于后续扩展更复杂的审计或策略。
