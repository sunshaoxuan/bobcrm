...
### 任务清单
- [x] 创建 `IconSelector.razor` 组件
- [x] 列出 Ant Design 常用图标（120+ 个常用图标）
- [x] 实现图标搜索功能
- [x] 实现图标预览
- [x] 集成到 MenuManagement.razor
- [ ] 支持自定义图片 URL（可选，未实现）
...
### 当前问题
- ✅ GUIDE-06 提到导入/导出，现已实现
- ✅ 可以快速复制菜单配置到测试/生产环境

### 任务清单
- [x] 后端 API:
  - [x] GET `/api/access/functions/export` - 导出菜单树为 JSON
  - [x] POST `/api/access/functions/import` - 导入菜单树
- [x] 前端 UI:
  - [x] 添加"导出"按钮（下载 JSON 文件）
  - [x] 添加"导入"按钮（上传 JSON 文件）
  - [x] 导入前预览和验证
- [x] 错误处理：
  - [x] 检测冲突的功能码
  - [x] 提供合并策略选项
- [x] i18n 资源：添加所有相关的多语言键
...
|--------|----------|----------|----------|------|
| **M1: 文档完善** | T1 | 菜单编辑器使用指南发布 | Week 1 | ✅ 已完成 |
