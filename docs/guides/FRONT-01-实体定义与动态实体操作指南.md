...
枚举字段允许在实体中定义有限的选项列表，提供更好的数据一致性和用户体验。

#### 4.3.1 创建枚举定义
1. 进入 **系统管理** → **枚举管理** (`/system/enums`)
2. 点击 **新建枚举** 按钮
3. 填写基本信息：
   - **枚举代码**：唯一标识符（例如：`customer_type`），创建后不可修改
   - **显示名称**：多语言名称（中文/英文/日文）
   - **描述**：可选的说明文字
   - **是否启用**：禁用的枚举不可在实体中使用
4. 添加枚举选项：
   - **选项值**：存储到数据库的实际值（例如：`ENTERPRISE`）
   - **显示名称**：用户界面显示的文本（支持多语言）
   - **颜色标签**：可选的颜色标记（`red`, `blue`, `green`等）
   - **图标**：可选的图标名称（Ant Design图标）
   - **排序**：控制选项显示顺序
5. 点击 **保存** 完成创建
...
1. 在实体定义编辑器中添加新字段
2. **数据类型** 选择 **Enum**
3. 从 **枚举定义** 下拉列表中选择已创建的枚举（例如：`customer_type`）
4. 可选配置：
   - **是否必填**：控制该字段是否必须填写
   - **是否多选**：启用后可选择多个枚举值
5. 保存实体定义并发布
...
#### 4.3.3 枚举字段在UI中的表现
- **表单输入**：自动渲染为下拉选择器（`EnumSelector`组件）
  - 支持搜索和过滤
  - 显示颜色标签和图标
  - 单选/多选模式自动适配
- **列表显示**：`DataGrid` 自动解析枚举值为显示名称
  - 显示对应的多语言文本
  - 应用颜色标签样式
  - 多选枚举显示为逗号分隔的标签列表
...
| `status` | 通用状态 | ACTIVE(激活), INACTIVE(未激活), PENDING(待处理), ARCHIVED(已归档) |
| `customer_type` | 客户类型 | INDIVIDUAL(个人), ENTERPRISE(企业), PARTNER(合作伙伴), VIP(VIP客户) |
| `gender` | 性别 | MALE(男), FEMALE(女), OTHER(其他) |
| `boolean` | 是/否 | TRUE(是), FALSE(否) |

#### 4.3.5 枚举使用注意事项
1. **枚举代码不可修改**：创建后枚举代码会被实体字段引用，修改会导致引用失效
2. **选项值不可修改**：选项值存储在数据库中，修改会影响历史数据
3. **禁用vs删除**：
   - 禁用枚举：已使用该枚举的实体不可发布，但不影响已发布的实体
   - 禁用选项：新数据不可选择，但历史数据仍可显示
4. **系统枚举保护**：标记为"系统枚举"的定义不可删除
5. **引用检查**：删除枚举前系统会检查是否被实体引用，有引用时无法删除
6. **多选枚举存储**：多选枚举值以JSON数组形式存储，例如：`["VALUE1", "VALUE2"]`
