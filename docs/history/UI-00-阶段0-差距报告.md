...
| Ant Design 默认主题（未显式 CSS 文件） | Buttons、Tables、Inputs 等 | 尚无覆盖，直接使用 Ant Design 主题色（#1677ff 等），与说明书色板冲突 |
| 内联样式（>150 处，示例：`src/BobCrm.App/Components/Shared/CustomerSider.razor:9-41`、`Components/Pages/Templates.razor:11-58`、`Components/Pages/FormDesigner.razor:35` 等） | 大量页面/组件直接写 `style="..."` | 1) 颜色/阴影/圆角/间距全部 hardcode，无法继承 Token；2) 与 8pt 栅格、容器三层语法不匹配；3) dark 模式无法复用；4) 难以统一 hover/focus 交互 |
| JS/TS 样式（`wwwroot/app.js`, `dragManager.js` 等） | 某些地方直接操作 DOM class | 未对接未来主题，仍写死颜色或缺少状态回调（需进一步在后续阶段核对） |

**关键差距总结**

1. **缺失 Design Tokens**：未提供 `bg/base`, `primary/600`, `shadow/raised` 等 CSS 变量，导致任何组件都无法引用说明书中的视觉语法。  
2. **不支持多主题**：`.theme-dark` 仅覆盖少数变量，缺少 blur/阴影/语义色映射，与说明书第 3.5 节相悖。  
3. **容器语法缺位**：没有 `glass-surface`, `elev-raised`, `section-container` 等基础类，页面只能用内联拼色，无法实现“容器 + 内容 + 状态”三层结构。  
4. **交互状态不统一**：未实现 `hover/active/focus/disabled` 状态类，更没有 `focus-ring`；按钮、输入、表格等使用 Ant Design 默认交互效果。  
5. **FormDesigner 专属样式老旧**：`app.css` 中的设计器样式依旧使用纯白/灰背景、单边框和默认阴影；拖拽句柄颜色 (#1890ff) 与 Calm 主题不符，也未考虑暗色模式。  
6. **可维护性低**：style 属性遍布 Razor，阻碍主题切换、可达性调整与后续 lint。
...
| --- | --- | --- |
| **App Shell** (`Components/Layout/MainLayout.razor`, `Components/Shared/AppHeader.razor`, `Components/Shared/CustomerSider.razor`) | 导航 & Chrome、Action（按钮/菜单）、Status（头像状态）、Drawer/Workspace | 使用 `theme.css` 旧主题 + 大量内联（`CustomerSider.razor:9-41`）；无可折叠/隐藏 API；无 glass 层与 blur，暗色模式缺乏 |
...
| **通知/反馈** (`Components/Shared/ToastHost?` 暂无) | Toast、Inline alert、Error boundary | 仅有 `blazor-error-boundary` 默认样式；缺乏统一 toast / empty state / skeleton 组件 |
