...
#### 状态
✅ 已完成 (2025-12-26)
- **Service**: 引入 `IJsInteropService` 与 `JsInteropService`，封装 `IJSRuntime`。
- **Safety**: 统一了 Try-Catch 逻辑，集成 Logger 与 Toast（可选）。
- **Refactor**: 迁移了 `AuthService` 与 `Login.razor` 核心流程。
- **I18n**: 已添加 `ERR_JS_INTEROP` 等基础资源键。
- **Commit**: `02f0479`
...
#### 状态
✅ 已完成 (2025-12-26)
- **Core**: 新增 `ErrorCodes` 统一错误码常量表，集中管理后端错误码。
- **Handler**: `GlobalExceptionHandler` 统一异常到 (HTTP Status, ErrorCode, `ERR_{CODE}`) 的映射与日志策略。
- **ApiErrors**: 统一输出 `ErrorResponse`，并补齐 Validation/Concurrency 等错误的标准错误码。
- **I18n**: 补齐 `ERR_{CODE}` 三语资源键（zh/ja/en），并通过脚本校验。
- **Tests**: 新增错误码去重/非空校验与异常映射校验（含 `ERR_{CODE}` 资源键存在性）。
- **Commit**: `41a1128`
...
#### 状态
✅ 已完成 (2025-12-26)
- **Strict (CI-only)**: 新增 `Directory.Build.props`，仅在 CI 环境启用 `TreatWarningsAsErrors=true`，避免影响本地开发体验。
- **Baseline**: 新增 `scripts/check-warning-baseline.ps1` + `warning-baseline.json`，基于实例级告警（code+file+line+col）冻结当前告警债务。
- **CI Gate**: 在 `ci.yml` 与 `verify-deployment.yml` 中执行基线脚本，阻断任何“新增告警实例”。
