...
| 目标覆盖率 | 90.00% | 项目验收标准 |
| 差距 | 7.09% | 约 700-1000 行核心代码 |

### 1.2 剩余未测试区域 (Gap Analysis)
根据 PLAN-27 的执行结果和文件浏览，主要盲区集中在以下"高复杂、低覆盖"区域：

1.  **动态实体运行时 (Dynamic Entity Runtime)**: 这是系统的核心引擎，负责处理实际的用户数据 (CRUD)，逻辑极其复杂，涉及 `DynamicEntityService` 和 `ReflectionPersistenceService`。目前测试很少。
2.  **模板与UI系统 (Template System)**: `TemplateService` 和 `DefaultTemplateGenerator` 包含大量未测试的生成和绑定逻辑。
3.  **认证与系统设置 (System & Auth)**: `AuthEndpoints` 的异常路径（锁定、刷新令牌失败）及 `SystemEndpoints`（枚举、菜单）覆盖不足。

---

## 二、测试策略

继续沿用 PLAN-27 验证成功的**"端点+服务"深度测试模式**。
