...
- [x] **系统实体迁移** (核心完成):
    ...
    - [x] 详见 [MIGRATION-01-系统实体模板化迁移记录](history/MIGRATION-01-系统实体模板化迁移记录.md)
    - [x] **已补偿**: UserRoleAssignment 组件（用户角色分配功能） - 2025-11-20
    - [x] **已补偿**: RolePermissionTree 组件（角色权限树功能） - 2025-11-20
    - [x] **已集成**: 将补偿组件嵌入User/Role详情模板 - 2025-11-20
    - [ ] **待迁移**: Organization 实体（已推迟，低优先级）

### 1.2 菜单与导航管理 (优先级提升)
> **说明**: 提升此模块优先级是为了完成权限系统的闭环。只有实现了可视化的菜单管理并与权限挂钩，后续的高级功能才能安全地交付给最终用户。

- [x] **动态枚举管理入口** - 2025-11-20:
    - [x] 在系统菜单中添加 `/system/enums` 入口（仅管理员可见）
    - [x] 权限码关联：`SYS.ENUM`
    - [x] 通过SystemMenuSeeder自动初始化菜单和权限
    - [x] 详见 [GUIDE-05-动态枚举系统使用指南](guides/GUIDE-05-动态枚举系统使用指南.md)
- [x] **可视化菜单编辑器** - 2025-11-20:
    - [x] 拖拽式菜单层级配置（before/after/into放置）
    - [x] 将菜单链接到动态实体或自定义页面（路由/模板双模式）
    - [x] **权限集成**: 通过FunctionNode.Code关联功能权限码
    ...
    - [x] 实施计划参考 [PROC-06-v0.6.0-开发计划](history/PROC-06-v0.6.0-开发计划.md) (已完成)

### 1.4 v0.8.0 开发重点 (已完成)
> **状态**: ✅ 完成
- [x] **架构重构**: TemplateService 提取。
- [x] **字段级安全控制**:
    - [x] 字段权限配置 UI（在角色管理中）
    - [x] API 响应字段过滤
    - [x] 表单运行时字段隐藏/禁用

### 1.5 v0.9.0 系统完善 (当前规划)
- [ ] **系统稳定性与测试**: 全面单元测试覆盖。
- [ ] **文档完善**: 开发者文档与用户手册。
- [ ] **UI/UX 统一**: 确保所有页面风格一致（Ant Design）。

---

## 📊 版本 1.5: 智能分析与报表引擎
**目标**: 构建一个独立于事务处理 (CRUD) 的高性能数据分析与可视化引擎。该引擎负责从数据聚合、钻取到前端展现的全过程。

### 2.1 独立查询引擎
- [ ] **查询定义器**: 独立于实体定义的查询模型，支持聚合、分组、计算字段。
- [ ] **高性能数据层**: 绕过标准 EF Core 实体追踪，使用只读投影或 Dapper 进行高效数据读取。
- [ ] **数据权限集成**: 深度集成现有的 RBAC 和数据范围规则，确保分析数据的安全性。

### 2.2 可视化与交互
- [ ] **仪表盘设计器**: 专业的拖拽式布局工具，支持网格系统。
- [ ] **图表组件库**:
    - [ ] 基础图表: 折线、柱状、饼图、雷达图。
    - [ ] 高级组件: 数据透视表、KPI 指标卡、漏斗图。
- [ ] **交互能力**:
    - [ ] **数据钻取 (Drill-down)**: 从汇总数据点击进入详细记录。
    - [ ] **联动过滤**: 仪表盘级别的全局筛选器。

### 2.3 报表管理
- [ ] **报表快照**: 定时生成报表数据快照。
- [ ] **导出与分享**: PDF/Excel 导出，报表订阅。
...
## ⚙️ 版本 2.0: 工作流自动化引擎 (Elsa Workflow 集成)
