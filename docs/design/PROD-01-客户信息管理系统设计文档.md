...
#### 6. **设计态渲染与属性编辑器架构（v0.5.5 重构，2025-11-06）**
...
**❌ 重构前的问题**：
- 每个容器都有独立的 PropertyPanel 组件（5个文件，~250 行重复代码）
- 大量重复的 `<label>` + `<Input>` 结构
- 添加新 Widget 需要创建新的 PropertyPanel 组件

**✅ 重构后的架构**：

**核心文件**：
- `Models/Designer/PropertyMetadata.cs` - 属性元数据定义
  ```csharp
  class WidgetPropertyMetadata {
      string PropertyPath;          // 属性路径（支持嵌套）
      string Label;                 // 多语言键
      PropertyEditorType EditorType; // 编辑器类型
      int? Min, Max;               // 数值范围
      List<PropertyOption>? Options; // 下拉选项
      string? Group;               // 分组（多语言键）
      string? VisibleWhen;         // 条件显示
  }
  ```

- `Components/Designer/PropertyEditor.razor` - 通用属性编辑器
  - 根据元数据动态渲染输入控件
  - 支持 Text, Number, Boolean, Color, Select, Textarea
  - 支持条件显示（`VisibleWhen`）
  - 支持属性分组（`Group`）
  - 支持嵌套属性路径（如 `ContainerLayout.Gap`）

- **面向对象重构**：所有 15 个 Widget 类重写 `GetPropertyMetadata()` 方法
  ...
  public override List<WidgetPropertyMetadata> GetPropertyMetadata()
  {
      ...
      {
          new() { PropertyPath = "Columns", Label = "PROP_COLUMNS", ... },
          new() { PropertyPath = "Gap", Label = "PROP_GAP", ... }
      };
  }
  
  // TextboxWidget.cs (普通组件示例)
  public override List<WidgetPropertyMetadata> GetPropertyMetadata()
  {
      return new List<WidgetPropertyMetadata>
      {
          new() { PropertyPath = "Placeholder", Label = "LBL_PLACEHOLDER", ... },
          new() { PropertyPath = "MaxLength", Label = "LBL_MAX_LENGTH", ... }
      };
  }
  ```

- `Services/Designer/WidgetPropertyProvider.cs` - 极简适配器（仅1行逻辑）
  ```csharp
  public List<WidgetPropertyMetadata> GetProperties(DraggableWidget widget)
  {
      return widget.GetPropertyMetadata();  // 委托给 Widget 自己
  }
  ```

**重构成果**：
- ✅ 删除 5 个重复的 PropertyPanel 组件（~250 行）
- ✅ 封装原则：所有 15 个 Widget 是自己属性的提供者
- ✅ 单一职责：属性定义与类在一起
- ✅ 开闭原则：添加新 Widget 无需修改 Provider
- ✅ 统一体验：所有属性编辑器UI一致
- ✅ 多语言支持：62 个多语言键，覆盖所有 Widget 的属性
- ✅ 架构完整性：容器组件 + 普通组件全部实现

**设计原则总结**：
- ✅ **设计器与实体解耦** - FormDesigner不知道设计的是什么实体
- ✅ **渲染器与实体解耦** - PageLoader通过EntityType参数动态加载任何实体
- ✅ **单一职责** - 设计是设计，渲染是渲染，属性是属性，完全分离
- ✅ **高度复用** - 一个FormDesigner、一个PageLoader、一个PropertyEditor支持所有类型
- ✅ **组件化渲染** - 设计态和运行态都采用组件化架构，职责清晰
- ✅ **面向对象封装** - 每个 Widget 是自己的属性、渲染和行为的提供者
...
- 安全：生产密钥与连接串管理；最小权限；审计基本操作日志（后续）。
 ...
 - 端口与配置：后端可自定端口；默认 API 端口 5200；前端默认监听 3000，并通过配置 `Api:BaseUrl` 指向 API。
...
4. ✅ **重构成果**：
   ...
   - 删除TemplateDesigner.razor（已合并路由到FormDesigner）
   - 总代码量大幅减少，架构更清晰
   - 所有测试通过
